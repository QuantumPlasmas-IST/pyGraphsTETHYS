...
for filename in glob.glob("*.h5"):
	...
	aspect=f['Data'].attrs['Aspect ratio'][0]
	fatia=120
	plt.plot(Y/Y[-1],u[:,fatia]/u[data_group.attrs.get('Number of spatial points y')[0]/2,fatia],label=aspect)
	plt.xlabel(r'$y/W$')
	plt.ylabel(r'$u/v_0$')
	#plt.tight_layout()
